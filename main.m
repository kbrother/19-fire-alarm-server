...
function initialize_tensor(topic, msg, num_channel, window_len, rank)
	% Create tensors
	global init_tensor
	global tensor_window	
	global window_sum
...
function fill_tensor(topic, msg, num_channel, time_stamp)
	...
	for i=1:num_channel				
		...
		for j=1:num_id				
			...
			init_tensor.(topic){i}(idx_cnt, 6, time_stamp) = msg(i).co(idx_cnt);
			init_tensor.(topic){i}(idx_cnt, 7, time_stamp) = msg(i).co2(idx_cnt);			
			...
			tensor_window.(topic){i, time_stamp}(idx_cnt, 6) = msg(i).co(idx_cnt);
			tensor_window.(topic){i, time_stamp}(idx_cnt, 7) = msg(i).co2(idx_cnt);			
			...
			window_sum.(topic){i}(idx_cnt, 6) = window_sum.(topic){i}(idx_cnt, 6) + msg(i).co(idx_cnt);
			window_sum.(topic){i}(idx_cnt, 7) = window_sum.(topic){i}(idx_cnt, 7) + msg(i).co2(idx_cnt);	
		...
		if real_num_id.(topic){i} > 0
			...
			for j=1:num_id
				if msg(i).err(j) == 0
					...
					tensor_window.(topic){i, window_len}(idx_cnt, 6) = msg(i).co(j);
					tensor_window.(topic){i, window_len}(idx_cnt, 7) = msg(i).co2(j);	
					idx_cnt = idx_cnt + 1;
				end
			end
			%disp("here2");
	...
	if getCnt > window_len*num_topic		
		...
		for i=1:num_channel		
			if real_num_id.(topic){i} > 0
				...
				Rt = Yt.(topic){i} - Ythat;								
				cRt = huber(Rt./(sigma.(topic){i} + 1e-4)).*sigma.(topic){i};								
				...
				for n=1:2
					G{n} = G{n} * min(1, 0.1 * sqrt(R)/(norm(G{n}, 'fro') + 1e-4)); % What?
					U.(topic){i, n} = U.(topic){i, n} + 0.1 * G{n};
				end				
				G{3} = G{3} * min(1, 0.1 * sqrt(R)/(norm(G{3}, 'fro') + 1e-4)); % What?
				U_N = U_N + 0.1 * G{3};
	...
	if and(getCnt >= (window_len-1)*num_topic+1 , getCnt <= window_len*num_topic)		
		...
		for i=1:num_channel		
			...
			if real_num_id.(topic){i} > 0
				...
				[U_init, X_hat.(topic), O.(topic), ~] = sofia_init(init_tensor.(topic){i}, omega_temp, R, lambda1, lambda3);							
